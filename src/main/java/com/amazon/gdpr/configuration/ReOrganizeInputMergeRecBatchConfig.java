	...
	public JdbcCursorItemReader<GdprDepersonalizationInput> gdprMergeRecDepersonalizationDBreader(@Value("#{jobParameters[RunId]}") long runId,
		...
		if(GlobalConstants.JOB_INPUT_CAN_RECORD.equalsIgnoreCase(recordType)) {
			gdprDepersonalizationDataFetch =  "SELECT DISTINCT GD.CANDIDATE__C, GD.BGC_APPLICATION__c,GD.CANDIDATE_SFDC_ID__C, GD.CATEGORY__C, GD.COUNTRY_CODE__C, "
				...
				+ "AND GD.CREATEDDATE < RM.DATA_LOAD_DATE AND DL.STATUS='ACTIVE' AND (GD.CANDIDATE__C IS NULL OR GD.CANDIDATE__C='') AND GD.CANDIDATE_SFDC_ID__C IS NOT NULL AND  (GD.BGC_STATUS__C IS NULL OR GD.BGC_STATUS__C='') AND RM.RUN_ID = "+runId 
				+ " ORDER BY GD.CANDIDATE__C";
		} else {
			gdprDepersonalizationDataFetch = "SELECT DISTINCT GD1.CANDIDATE__C, GD.CANDIDATE_SFDC_ID__C,GD.COUNTRY_CODE__C, 'EMPLOYEE' RECORD_TYPE, "
				...
				+ "WHERE  (GD.CREATEDDATE >= DL.LAST_DATA_LOADED_DATE OR GD.LASTMODIFIEDDATE >= DL.LAST_DATA_LOADED_DATE) "
				+ "AND GD.CREATEDDATE < RM.DATA_LOAD_DATE AND DL.STATUS='ACTIVE' AND (GD.CANDIDATE__C IS NULL OR GD.CANDIDATE__C='') AND GD.CANDIDATE_SFDC_ID__C IS NOT NULL AND RM.RUN_ID = "+runId 
				+ " ORDER BY GD.CANDIDATE__C";
		}
